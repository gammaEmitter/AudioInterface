...
int main(int argc, char** argv) {
    if (!argv[1]) return -1;
    AAllocator alloc {};
    init_allocator(alloc, 65536);
    try {
        std::string file  = argv[1];
        WavTool::RiffWAV* wav = WavTool::readWAV(alloc, file);
        ...
        std::cout << "Data Size: " << wav->len_data << " samples" << std::endl;
        std::cout << "Duration: " << ((float) wav->len_data / (float) wav->sampleRate) << " seconds" << std::endl;
